...
class rsa
{
    ...
    public:
        ...
        void generarClaves(int nbits){
            ...
            clavesPrivadas.open("clavesPrivadas.txt");
            srand(time(0));
            //para p
            while(true){
                p=GenPrime_ZZ(nbits);//funcion del NTL para generar primos
            ...
            //para q
            while(true){
                q=GenPrime_ZZ(nbits);//funcion del NTL para generar primos
            ...
            //generando e
            while(true){
                e=mod(RandomBits_ZZ(nbits),fiN);
                if(mcd(e,fiN)==ZZ(1)){
                    break;
                }
            }
            d=mod(inversa(e,fiN),fiN);

            directorioPublico << "\nClave n: " << n << "\nClave e: " << e << endl;
            clavesPrivadas << "p: "<< p <<"\n"<< "q: "<< q <<"\n"<< "n: "<< n <<"\n"<< "fiNn: "<< fiN <<"\n"<< "e: "<< e <<"\n"<< "d: "<< d << endl ;

            directorioPublico.close();
